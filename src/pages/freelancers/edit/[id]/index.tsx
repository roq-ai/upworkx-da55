...
import { useRoqClient, useFreelancerFindFirst } from 'lib/roq';
...
function FreelancerEditPage() {
  ...
  const { data, error, isLoading, mutate } = useFreelancerFindFirst(queryParams, {}, { disabled: !id });
  const [formError, setFormError] = useState(null);

  const handleSubmit = async (values: FreelancerInterface, { resetForm }: FormikHelpers<any>) => {
    setFormError(null);
    try {
      const updated = await roqClient.freelancer.update({
        data: values as RoqTypes.freelancer,
        where: {
          id,
        },
      });

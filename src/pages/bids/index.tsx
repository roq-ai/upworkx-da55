...
import { useRoqClient, useBidFindManyWithCount } from 'lib/roq';
...
export function BidListPage(props: BidListPageProps) {
      ...
      convertQueryToPrismaUtil(
        {
          ...
          searchTermKeys: ['message.contains'],
          ...params.filters,
        },
        'bid',
      ),
  ...
  const { data, error, isLoading, mutate } = useBidFindManyWithCount({
  ...
  const handleDelete = async (id: string) => {
    setDeleteError(null);
    try {
      await roqClient.bid.delete({ where: { id } });
      await mutate();
    } catch (error) {
  ...
  const columns: ColumnType[] = [
    { id: 'amount', header: 'amount', accessorKey: 'amount' },
    {
      ...
      accessorKey: 'proposed_completion_date',
      cell: ({ row: { original: record } }: any) =>
        record?.proposed_completion_date ? format(record?.proposed_completion_date, 'dd-MM-yyyy') : '',
    },

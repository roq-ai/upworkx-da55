...
export function ReviewListPage(props: ReviewListPageProps) {
  ...
  const columns: ColumnType[] = [
    ...
    hasAccess('user', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)
      ? {
          id: 'user_review_reviewer_idTouser',
          header: 'User Review Reviewer Id Touser',
          accessorKey: 'user_review_reviewer_idTouser',
          cell: ({ row: { original: record } }: any) => (
            <Link
              as={NextLink}
              onClick={(e) => e.stopPropagation()}
              href={`/users/view/${record.user_review_reviewer_idTouser?.id}`}
            >
              {record.user_review_reviewer_idTouser?.email}
            </Link>
          ),
        }
      : null,
    hasAccess('user', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)
      ? {
          id: 'user_review_reviewee_idTouser',
          header: 'User Review Reviewee Id Touser',
          accessorKey: 'user_review_reviewee_idTouser',
          cell: ({ row: { original: record } }: any) => (
            <Link
              as={NextLink}
              onClick={(e) => e.stopPropagation()}
              href={`/users/view/${record.user_review_reviewee_idTouser?.id}`}
            >
              {record.user_review_reviewee_idTouser?.email}
            </Link>
          ),
        }
      : null,
